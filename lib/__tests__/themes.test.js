...
describe("THEMES", () => {
  it("has six themes with unique ids and a valid default", () => {
  ...
  it("gives every theme a label, a mood and two hex swatches", () => {
...
describe("themeInitScript", () => {
  ...
  it("falls back on the OS preference: dark gets blueprint, light gets the default", () => {
    expect(run({ dark: true })).toBe("blueprint");
    expect(run({ dark: false })).toBe(DEFAULT_THEME);
  });

  it("ignores a saved value that is not a theme", () => {
    expect(run({ saved: "hotpink" })).toBe(DEFAULT_THEME);
